...
function getSpiralMatrix(size) {
  ...
  while (item <= size * size) {
    for (let i = oneColumn; i <= lastColumn; i += 1) {
      res[oneRow][i] = item;
      item += 1;
    }
    oneRow += 1;
    for (let i = oneRow; i <= lastRow; i += 1) {
      res[i][lastColumn] = item;
      item += 1;
    }
    lastColumn -= 1;
    for (let i = lastColumn; i >= oneColumn; i -= 1) {
      res[lastRow][i] = item;
      item += 1;
    }
    lastRow -= 1;
    for (let i = lastRow; i >= oneRow; i -= 1) {
      res[i][oneColumn] = item;
      item += 1;
    }
    oneColumn += 1;
  }
...
function rotateMatrix(matrix) {
  ...
  for (let i = 0; i < len / 2; i += 1) {
    for (let j = i; j < len - i - 1; j += 1) {
      ...
      const resolt = matrix;
      resolt[i][j] = resolt[len - j - 1][i];
      resolt[len - j - 1][i] = resolt[len - i - 1][len - j - 1];
      resolt[len - i - 1][len - j - 1] = resolt[j][len - i - 1];
      resolt[j][len - i - 1] = item;
    }
  }
  return matrix;
}
...
function getNearestBigger(number) {
  ...
  digits = digits.splice(i, digits.length + 1);
  digits.sort();
